...
patches-own[
  ...
  growth               ;simulates growth of food
]
...
to setup
  ...
  create-predators predators-number
  [
    set color red
    set size 7
    setxy random-xcor random-ycor
...
to go
  ...
  ask preys [ set energy energy - 1 ]
  ask predators [ set energy energy - 1 ]
  move
...
to move
  ...
  repeat 5 [ ask predators [ fd 0.3 ] display ]
  ;; for greater efficiency, at the expense of smooth
  ;; animation, substitute the following line instead:
;  ask preys [ fd 1 ]
;  ask predators [ fd 1.3 ]
end
...
to flock  ;prey procedure
  ...
  if any? herd[
    ...
    ifelse distance nearest-neighbor < avoidance-distance[
      avoid
    ][ align
       cohere]
  ]
  flee
  if energy < 70 [ find-food ]
;  if energy = 0 [ die ]
end

;;Find flockmates within range of vision
to find-herd  ;prey procedure
  set herd other preys in-cone prey-scope-vision prey-range-vision
end

;;Find the closest neighbot within the herd
to find-nearest-neighbor  ;prey procedure
  set nearest-neighbor min-one-of herd [distance myself]
end

;;Prevent preys from colliding with their herdmates
to avoid  ;prey procedure
  turn-away ([heading] of nearest-neighbor) max-avoid-turn
end
...
;;Make preys run away from predators
to flee  ;prey procedure
  let threads predators in-cone prey-scope-vision predator-range-vision
  if any? threads [
    let thread min-one-of threads [ distance myself ]
    set heading ([heading] of thread)
    turn-away heading 10
  ]
end
...
;;Average of the herd heading, using trigonometry.
;;Average of 1 and 359 equals 0.
...
to-report average-heading-towards-herdmates  ;prey procedure
  ;"towards myself" + 180 gives us the heading from
  ;this turtle to others
  let x-component mean [sin (towards myself + 180)] of herd
  let y-component mean [cos (towards myself + 180)] of herd
  ifelse x-component = 0 and y-component = 0
    [ report heading ]
    [ report atan x-component y-component ]
end

;;Predators movement towards preys, eat them if
;;possible
to hunt  ;predator procedure
  set herd preys in-cone predator-scope-vision predator-range-vision
  ;Persuit prey
  if any? herd[
    set nearest-neighbor min-one-of herd [distance myself]
...
;;Predators eat prey within reachable range
...
;;Change heading of the turtle torwards his flockmates
